...
def log(msg: str) -> None:
    print(f"[run-directional-from-config] {time.strftime('%H:%M:%S')} {msg}", flush=True)
...
def spawn_controller(uuid: str) -> subprocess.Popen[str]:
    cmd = [
        sys.executable,
        "entrypoints/run_directional.py",
        "--biorobot-uuid",
        uuid,
        "--bootstrap-mqtt-host",
        MQTT_HOST,
    ...
    log(f"starting controller for uuid={uuid}")
...
def main() -> None:
    ...
    if not uuids:
        raise RuntimeError("No robot UUIDs found in retained config")

    log(f"found {len(uuids)} configured robots: {uuids}")
    ...
    while running:
        for child in children:
            rc = child.poll()
            if rc is not None:
                raise RuntimeError(f"Directional controller exited unexpectedly with code {rc}")
        time.sleep(1.0)
